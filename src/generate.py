...
class Generate:
    def __init__(self):
        ...
        self._reverse_input_char_index = ''
        self._reverse_target_char_index = ''
    ...
    def Model(latent_dim, self):

        # define the model
        # 다중 인풋 
        model = Model([self._encoder_inputs, self._decoder_inputs], self._decoder_outputs)
        ...
        self._encoder_model = Model(self._encoder_inputs, self._encoder_states)

        decoder_state_input_h = Input(shape=(latent_dim,))
        decoder_state_input_c = Input(shape=(latent_dim,))
        decoder_states_inputs = [decoder_state_input_h, decoder_state_input_c]

        self._decoder_outputs, state_h, state_c = self._decoder_lstm(
        ...
        self._decoder_model = Model(
            ...
            [self._decoder_outputs] + decoder_states)
    ...
    def backPropagation(self):
        ...
        self._reverse_input_char_index = dict(
            (i, char) for char, i in self._input_token_index.items())
        self._reverse_target_char_index = dict(
            (i, char) for char, i in self._target_token_index.items())
    ...
    def decode_sequence(input_seq, diversity, self):
        ...
        target_seq[0, 0, self._target_token_index['\t']] = 1.
        ...
        while not stop_condition:
            ...
            sampled_token_index = self.sample(output_tokens[0, -1, :], diversity)
            sampled_char = self._reverse_target_char_index[sampled_token_index]
            decoded_sentence += sampled_char

            # Exit condition: either hit max length
            # or find stop character.
            if len(decoded_sentence) > self._max_decoder_seq_length:
                stop_condition = True
    ...
    def headAndTail(maxlen, length, text):
        ...
        return prefix, generated, head, tail


    def whileSmode(smode, num_line, max_num_line, generated, k, prefix, self):

        while (num_line < max_num_line and k < 150):
            k = k + 1
            # nosample: 학습된 분포에 직접 의존
            if (smode is 'nosample'):
                next_char = self.decode_sequence(prefix, 1)
            # sample: prefix sequence가 주어지면 다음 문자를 sampling
            if (smode is 'sample'):
                next_char = self.decode_sequence(prefix, 1.2)
    ...
    def synthesis(self, maxlen, max_num_line, text, gmode='g1', smode='nosample'):
        ...
        if (gmode is 'g1'):
            prefix, generated, head, tail = self.headAndTail(maxlen, length, text)
            ...
            generated = \
                self.whileSmode(smode, num_line, max_num_line, generated, k, prefix)
        ...
        if (gmode is 'g2'):
            for _ in range(2):
                prefix, generated, head, tail = \
                    self.headAndTail(maxlen, length, text, prefix)
                cut_index = tail.find(';') + 1
                tail = tail[cut_index:]

                num_line = 0
                k = 0
                half_max_num_line = max_num_line/2
                
                generated = \
                    self.whileSmode(smode, num_line, half_max_num_line, generated, k, prefix)
                text = head + generated + tail
        ...
        if (gmode is 'g3'):
            prefix, generated, head, tail = \
                self.headAndTail(maxlen, length, text)
...
def generate(seed_path, data_path):
    ...
    g.vectorizeData(data_path, num_samples)
    g.relatedEncoder(latent_dim)
    g.relatedDecoder(latent_dim) 
    g.Model(latent_dim)
    g.backPropagation()
    ...
    for file in files:
        ...
        try:
            ...
            text = g.synthesis(maxlen, max_num_line, text, generation_strategy, sampling_method)
